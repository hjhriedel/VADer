...
class Model():
    ...
    def build_model(self, shape): 
        ...
        c1 = Conv2D(n_filters, kernel_size = self.config.model.kernel_start, padding="same", activation='relu')(bn)
        
        for i in range(self.config.model.encode_layer):
            c1 = self._conv2d_block(c1, n_filters, i, kernel_size=kernel_mid)
        n_filters *= 2
        c2 = MaxPool2D()(c1) if shape[-2] > 2**0 else MaxPool2D(pool_size=(pool,1))(c1)

        if pooling_steps >= 2:
            for i in range(self.config.model.encode_layer):
                c2 = self._conv2d_block(c2, n_filters, i, kernel_size=kernel_mid)
            n_filters *= 2
        ...
        if pooling_steps >= 3:
            for i in range(self.config.model.encode_layer):
                c3 = self._conv2d_block(c3, n_filters, i, kernel_size=kernel_mid)
            n_filters *= 2
        ...
        if pooling_steps >= 4:
            for i in range(self.config.model.encode_layer):
                c4 = self._conv2d_block(c4, n_filters, i, kernel_size=kernel_mid)
            n_filters *= 2
        ...
        if pooling_steps >= 5:
            for i in range(self.config.model.encode_layer):
                c5 = self._conv2d_block(c5, n_filters, i, kernel_size=kernel_mid)
            n_filters *= 2
        ...
        if pooling_steps == 6:
            for i in range(self.config.model.encode_layer):
                c6 = self._conv2d_block(c6, n_filters, i, kernel_size=kernel_mid)
            n_filters *= 2
            c7 = MaxPool2D()(c6) if shape[-2] > 2**5 else MaxPool2D(pool_size=(pool,1))(c6)
        
        for i in range(self.config.model.bottleneck_layer):
            c7 = self._conv2d_block(c7, n_filters, i, kernel_size=kernel_mid)
        n_filters /= 2
        ...
        c7 = GroupNormalization(int(n_filters/16))(c7)
        c8 = Conv2D(n_filters, kernel_size = self.config.model.kernel_end, padding="same", activation='relu')(c7)
        outputs = Conv2D(1, kernel_size = self.config.model.kernel_end, padding="same", activation='sigmoid')(c8)
