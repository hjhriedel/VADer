...
class DataLoader():
    ...
    def get_train_data(self,shuffle=True):
        if shuffle:
            return self.trainDS.shuffle(buffer_size=64, reshuffle_each_iteration=True).apply(tf.data.experimental.prefetch_to_device('/gpu:0', buffer_size=tf.data.AUTOTUNE))#.prefetch(tf.data.AUTOTUNE)
        else:
            return self.trainDS.apply(tf.data.experimental.prefetch_to_device('/gpu:0', buffer_size=tf.data.AUTOTUNE))#.prefetch(tf.data.AUTOTUNE)
        
    def get_validation_data(self,shuffle=False):
        if shuffle: 
            return self.valDS.shuffle(buffer_size=64, reshuffle_each_iteration=True).apply(tf.data.experimental.prefetch_to_device('/gpu:0', buffer_size=tf.data.AUTOTUNE))#.prefetch(tf.data.AUTOTUNE)
        else:
            return self.valDS.apply(tf.data.experimental.prefetch_to_device('/gpu:0', buffer_size=tf.data.AUTOTUNE))#.prefetch(tf.data.AUTOTUNE)

    def get_testing_data(self):
        return self.testDS.prefetch(tf.data.AUTOTUNE)
    ...
    def _list_to_dataset(self, X, Y, buckets):
        ...
        ds = ds.bucket_by_sequence_length(
                ...
                bucket_batch_sizes=[self.batch_size,]*(len(buckets)+1))
        return ds     
