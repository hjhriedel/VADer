...
class DataLoader():
    ...
    def preprocess(self, val_size=0.2, test_size=0.1): 
        ...
        os.makedirs(os.path.join(self.config.callbacks.checkpoint_dir, self.timestamp, ''))
        with open(os.path.join(self.config.callbacks.checkpoint_dir, self.timestamp, 'test_names.ob'), "wb") as fp:
            pickle.dump(nameTest, fp)
    ...
    def get_train_data(self,shuffle=True):
        if shuffle:
            return self.trainDS.shuffle(buffer_size=250)
        else:
            return self.trainDS
        
    def get_validation_data(self,shuffle=False):
        if shuffle: 
            return self.valDS.shuffle(buffer_size=250)
        else:
            return self.valDS
